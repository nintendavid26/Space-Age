...
namespace Adnc.SkillTree.Example.MultiCategory {
	public class SkillMenu : MonoBehaviour {
		...
		public void ShowCategory (SkillCategoryBase category) {
			...
			categoryName.text = string.Format("{0}: ", category.displayName, category.skillLv);
			ClearDetails();
		...
		public void ShowNodeDetails (SkillNode node) {
			...
			sidebarTitle.text = string.Format("{0}:", skillCollection.displayName, skillCollection.SkillIndex + 1);
		...
		void UpdateSkillPoints () {
			skillOutput.text = "Money: " + skillTree.skillPoints;
		}
